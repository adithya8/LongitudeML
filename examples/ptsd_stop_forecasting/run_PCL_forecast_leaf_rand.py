...
if __name__ == '__main__':
    ...
    for key in datasetDict: datasetDict[key] = datasetDict[key].with_format('torch')
    ...
    if args.model_type == 'baseline':
        if args.custom_model in ['linear', 'linear_subscales', 'linear_subscales_z']:
        ...
        elif args.custom_model in ['boe_lang_z']:
            lang_model = BoELinear(input_size=args.input_size, num_classes=args.num_classes,
                                            num_outcomes=args.num_outcomes, output_dropout=args.output_dropout,
                                            max_len=args.max_len)
            model = BSLN_ARCHS[args.custom_model](lang_model)
            ...
            if args.lang_weights_path is not None:
                ...
                langAR.linear.weight.data = langAR_weights['state_dict']['model.langAR_model.linear.weight']
                langAR.linear.bias.data = langAR_weights['state_dict']['model.langAR_model.linear.bias']
