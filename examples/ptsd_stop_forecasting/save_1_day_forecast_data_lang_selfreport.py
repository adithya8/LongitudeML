...
def merge_datasets(long_lang_features, long_outcomes):
    ...
    for seq_id in mutual_seq_ids:
        ...
        # if seq_id == 3333: import pdb; pdb.set_trace()
...
def concatenate_features(feature_dict1, feature_dict2):
    """
    ...
    """
    concatenated_features = {"seq_id": [], "embeddings": [], "num_tokens": [], "time_ids": [], "infill_lang_mask": []}
    def unionize_time_ids(feat1_instance, feat2_instance):
        """
            Returns the union of time_ids between lang and outcomes instances.
        """
        merged_instance = {"seq_id": feat1_instance['seq_id'], "embeddings": [], "num_tokens": [], "lang_time_ids": [], "infill_lang_mask": []}
        ...
        for time_id in range(min(union_time_ids), max(union_time_ids)+1):
            if time_id in feat1_time_ids and time_id in feat2_time_ids:
                ...
                merged_embeddings = feat1_instance['embeddings'][feat1_instance['time_ids'].index(time_id)] + feat2_instance['embeddings'][feat2_instance['time_ids'].index(time_id)]
                merged_num_tokens = feat2_instance['num_tokens'][feat2_instance['time_ids'].index(time_id)]
                merged_instance["embeddings"].append(merged_embeddings)
                merged_instance["num_tokens"].append(merged_num_tokens)
                merged_instance["lang_time_ids"].append(time_id)
                infill_lang_mask = (feat1_instance['infill_lang_mask'][feat1_instance['time_ids'].index(time_id)]) \
                                    & (feat2_instance['infill_lang_mask'][feat2_instance['time_ids'].index(time_id)])
                merged_instance["infill_lang_mask"].append(infill_lang_mask)
                ...
                merged_embeddings = feat1_instance['embeddings'][feat1_instance['time_ids'].index(time_id)] + feat2_embs_infill
                merged_num_tokens = feat1_instance['num_tokens'][feat1_instance['time_ids'].index(time_id)]
                merged_instance["embeddings"].append(merged_embeddings)
                merged_instance["num_tokens"].append(merged_num_tokens)
                merged_instance["lang_time_ids"].append(time_id)
                infill_lang_mask = feat1_instance['infill_lang_mask'][feat1_instance['time_ids'].index(time_id)] & 1
                merged_instance["infill_lang_mask"].append(infill_lang_mask)
                merged_instance["infill_lang_mask"].append(0)
                prev_feat1_embs = feat1_instance['embeddings'][feat1_instance['time_ids'].index(time_id)]
                ...
                merged_embeddings = feat1_embs_infill + feat2_instance['embeddings'][feat2_instance['time_ids'].index(time_id)]
                merged_num_tokens = feat2_instance['num_tokens'][feat2_instance['time_ids'].index(time_id)]
                merged_instance["embeddings"].append(merged_embeddings)
                merged_instance["num_tokens"].append(merged_num_tokens)
                merged_instance["lang_time_ids"].append(time_id)
                infill_lang_mask = feat1_instance['infill_lang_mask'][feat2_instance['time_ids'].index(time_id)] & 1
                merged_instance["infill_lang_mask"].append(infill_lang_mask)
                ...
                feat1_embs_infill = prev_feat1_embs if time_id > min_feat1_time_id else np_zeros_like(feat1_instance['embeddings'][0]).tolist()
                feat2_embs_infill = prev_feat2_embs if time_id > min_feat2_time_id else np_zeros_like(feat2_instance['embeddings'][0]).tolist()
                merged_embeddings = feat1_embs_infill + feat2_embs_infill
                merged_num_tokens = 0
                merged_instance["embeddings"].append(merged_embeddings)
                merged_instance["num_tokens"].append(merged_num_tokens)
                merged_instance["lang_time_ids"].append(time_id)
                merged_instance["infill_lang_mask"].append(1) # If both time_ids are missing, then mask is set 1 
                prev_feat1_embs = feat1_embs_infill
                prev_feat2_embs = feat2_embs_infill
        return merged_instance        
        
    mutual_seq_ids = set(feature_dict1['seq_id']) & set(feature_dict2['seq_id'])
    for seq_id in mutual_seq_ids:
        feat1_seqid_idx = feature_dict1['seq_id'].index(seq_id)
        feat1_instance = feature_dict1[feat1_seqid_idx]
        feat2_seqid_idx = feature_dict2['seq_id'].index(seq_id)
        feat2_instance = feature_dict2[feat2_seqid_idx]
        merged_instance = unionize_time_ids(feat1_instance, feat2_instance)
        concatenated_features["seq_id"].append(seq_id)
        concatenated_features["embeddings"].append(merged_instance["embeddings"])
        concatenated_features["num_tokens"].append(merged_instance["num_tokens"])
        concatenated_features["time_ids"].append(merged_instance["lang_time_ids"])
        concatenated_features["infill_lang_mask"].append(merged_instance["infill_lang_mask"])
...
if __name__ == '__main__':
    ...
    long_sr_features = long_sr_features.map(create_lang_mask)
    ...
    merged_dataset = merge_datasets(long_comb_features, long_outcomes)
