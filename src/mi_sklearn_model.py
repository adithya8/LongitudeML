...
class AutoRegressiveSklearnBase:
    ...
    def select_features(self, batch_dict: Dict[str, torch.Tensor], args) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        print(f"[DEBUG] Stage 1 - After extracting from batch_dict:")
        ...
        if padding_len > 0:
            ...
            print(f"[DEBUG] Stage 2 - After padding (padding_len={padding_len}):")
            print(f"  X_3d_padded shape: {X_3d_padded.shape}, mask_3d_padded shape: {mask_3d_padded.shape}")
        else:
            X_3d_padded = X_3d
            mask_3d_padded = mask_3d
            print(f"[DEBUG] Stage 2 - No padding needed (max_len=1)")
        ...
        print(f"[DEBUG] Stage 3 - After unfold (before permute):")
        print(f"  X_unfolded shape: {X_unfolded.shape}")
        ...
        print(f"[DEBUG] Stage 3b - After permute:")
        print(f"  X_unfolded shape: {X_unfolded.shape}")
        ...
        print(f"[DEBUG] Stage 4 - After flattening windows:")
        print(f"  X_3d_flattened shape: {X_3d_flattened.shape}")
        
        # Note: The mask should remain unchanged - unfolding only affects features (X), not targets (y) or validity (mask)
        # The original mask correctly indicates which outcomes are valid at each timestep
        # The sklearn training will handle invalid outcomes when flattening via reshape_for_sklearn
        print(f"[DEBUG] Stage 5 - Mask unchanged:")
        print(f"  mask_3d shape: {mask_3d.shape}, y_3d shape: {y_3d.shape}")
        print(f"  Valid outcomes: {mask_3d.sum().item()}/{mask_3d.numel()}")
        
        return X_3d_flattened, y_3d, mask_3d
...
# ============================================================================
# Bag of Embeddings (BoE) Sklearn Models with History Windowing
# ============================================================================

class BoESklearnBase:
    """
    Base class for Bag of Embeddings (BoE) sklearn models that use history windowing.
    
    This class implements the averaging logic in `select_features()` similar to
    `BoELinear` in `mi_model.py`. It handles:
    - Left padding with zeros (causal padding)
    - Sliding window unfolding
    - Averaging embeddings in each window (instead of concatenating)
    
    Unlike `AutoRegressiveSklearnBase`, this class averages the embeddings in each
    window, resulting in feature vectors of size `input_dim` (not `max_len * input_dim`).
    This factors history but doesn't model temporal dynamics.
    ...
    def select_features(self, batch_dict: Dict[str, torch.Tensor], args) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Select features with Bag of Embeddings (BoE) history windowing.
        
        This method:
        1. Extracts embeddings from batch_dict
        2. Pads on the left with (max_len - 1) zeros for causal sliding
        3. Unfolds to create sliding windows of size max_len
        4. Averages embeddings in each window: (B, T, max_len, D) -> (B, T, D)
        ...
        Returns:
            X_3d: (batch_size, seq_len, input_dim) tensor - averaged features
            y_3d: (batch_size, seq_len, num_outcomes) tensor - targets (unchanged)
            mask_3d: (batch_size, seq_len, num_outcomes) tensor - unchanged
        """
        # Get max_len from args
        max_len = getattr(args, 'max_len', 1)
        if max_len < 1:
        ...
        # Extract embeddings (same logic as AutoRegressiveSklearnBase)
        embeddings_key = None
        preferred_keys = [
            "embeddings_lang_z",
            "embeddings_lang",
        ...
        print(f"Using embeddings key: {embeddings_key} (with max_len={max_len} BoE history windowing)")
        ...
        batch_size, seq_len, input_dim = X_3d.shape
        print(f"[DEBUG] BoE Stage 1 - After extracting from batch_dict:")
        print(f"  X_3d shape: {X_3d.shape}, y_3d shape: {y_3d.shape}, mask_3d shape: {mask_3d.shape}")
        
        # Pad on the left for causal sliding
        if padding_len > 0:
            # Pad along sequence dimension (dim=1) with zeros on the left
            X_3d_padded = torch.nn.functional.pad(X_3d, (0, 0, padding_len, 0), mode='constant', value=0.0)
            print(f"[DEBUG] BoE Stage 2 - After padding (padding_len={padding_len}):")
            print(f"  X_3d_padded shape: {X_3d_padded.shape}")
        else:
            X_3d_padded = X_3d
            print(f"[DEBUG] BoE Stage 2 - No padding needed (max_len=1)")
        
        # Unfold to create sliding windows
        # unfold(dimension=1, size=max_len, step=1) creates windows of size max_len
        # Shape after unfold: (batch_size, new_seq_len, input_dim, max_len)
        X_unfolded = X_3d_padded.unfold(dimension=1, size=max_len, step=1)
        print(f"[DEBUG] BoE Stage 3 - After unfold (before permute):")
        print(f"  X_unfolded shape: {X_unfolded.shape}")
        
        # Permute to get (batch_size, new_seq_len, max_len, input_dim)
        X_unfolded = X_unfolded.permute(0, 1, 3, 2)  # Swap last two dims: (B, T', D, max_len) -> (B, T', max_len, D)
        print(f"[DEBUG] BoE Stage 3b - After permute:")
        print(f"  X_unfolded shape: {X_unfolded.shape}")
        ...
        # Average embeddings across the window dimension: (batch_size, seq_len, max_len, input_dim) -> (batch_size, seq_len, input_dim)
        # Since padded positions are zeros, dividing by max_len effectively averages only valid positions
        X_3d_averaged = X_unfolded.mean(dim=2)  # Average over window dimension
        print(f"[DEBUG] BoE Stage 4 - After averaging windows:")
        print(f"  X_3d_averaged shape: {X_3d_averaged.shape}")
        
        # Note: The mask should remain unchanged - averaging only affects features (X), not targets (y) or validity (mask)
        print(f"[DEBUG] BoE Stage 5 - Mask unchanged:")
        print(f"  mask_3d shape: {mask_3d.shape}, y_3d shape: {y_3d.shape}")
        print(f"  Valid outcomes: {mask_3d.sum().item()}/{mask_3d.numel()}")
        
        return X_3d_averaged, y_3d, mask_3d


class BoERidge(BoESklearnBase, Ridge):
    """
    Ridge regression model with Bag of Embeddings (BoE) history windowing.
    
    This model uses sliding windows of size `max_len` (from args.max_len) to create
    averaged feature vectors. Each timestep's features are the average of the current
    and previous (max_len - 1) timesteps' embeddings.
    
    Unlike `AutoRegressiveRidge`, this model averages embeddings instead of concatenating
    them, resulting in feature vectors of size `input_dim` (not `max_len * input_dim`).
    This factors history but doesn't model temporal dynamics - it treats the window as
    a bag of embeddings.
    
    Example:
        If max_len=7 and input_dim=64, each timestep will have features of size 64
        (average of 7 days of embeddings), compared to 448 for AutoRegressiveRidge.
    
    Usage:
        model = BoERidge(alpha=1.0, random_state=42)
        module = SklearnModule(args, model)  # args.max_len must be set
        trainer = SklearnTrainer(output_dir=..., logger=...)
        results = trainer.fit(module, train_dataloader, val_dataloader)
    """
