...
base_dict_user_level = dict(db="ptsd_stop", msg_table="whisper_transcripts_v1", feature_tables=["feat$dr_rpca_128_fb20$whisper_transcripts_v1$user_day_id"],
                 outcome_table="outcomes_user_level_v2", outcome_field="", timeid_field="day_id", correl_field="user_id", messageid_field="user_day_id")
...
base_dict = dict(db="ptsd_stop", msg_table="whisper_transcripts_v1", feature_tables=["feat$dr_rpca_128_fb20$whisper_transcripts_v1$user_day_id"],
                 outcome_table="outcomes_v2", outcome_field="", timeid_field="day_id", correl_field="user_id", messageid_field="user_day_id")

INPUT_DICT = {
    "PCL_avg": deepcopy(base_dict_user_level),
    "Q9_avg": deepcopy(base_dict_user_level),
    "StressEvents_avg": deepcopy(base_dict_user_level),
    "PCL": deepcopy(base_dict),
    "Q9": deepcopy(base_dict),
    "StressEvents": deepcopy(base_dict)
}
...
if __name__ == '__main__':
    ...
    dataDict, longtype_encoder = dlatk_dataGetter.combine_features_and_outcomes(outcomes_correl_field=None if 'avg' in args.outcome_field else 'user_day_id')
